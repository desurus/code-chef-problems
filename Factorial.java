...
* decimal form of number N!. They noticed that this function never decreases. If we have two numbers N1<N2,
* then Z(N1) <= Z(N2). It is because we can never "lose" any trailing zero by multiplying by any positive number.
* We can only get new and new zeros. The function Z is very interesting, so we need a computer program that can
* determine its value efficiently.
*
* Input
*
* There is a single positive integer T on the first line of input (equal to about 100000). It stands for the number
* of numbers to follow. Then there are T lines, each containing exactly one positive integer number
* N, 1 <= N <= 1000000000.
*
* Output
*
